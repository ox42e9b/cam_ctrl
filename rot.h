...
rot_reset()
{
    ...
    trans.pos = trans.max - trans.min;
      yaw.pos =   yaw.max - yaw.min; 
    pitch.pos = pitch.max - pitch.min;

    rot_set_speed(trans_p, 1500);
    rot_set_speed(yaw_p,   800);
    rot_set_speed(pitch_p, 800);

    rot_set_target(trans_p, trans.ini);
    rot_set_target(  yaw_p,   yaw.ini);
    rot_set_target(pitch_p, pitch.ini);
...
{ 
    ...
    trans.max_speed = 12500;
    yaw.max_speed   = 2000;
    pitch.max_speed = 1600;
